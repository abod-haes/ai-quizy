...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "border-input data-[placeholder]:text-muted-foreground [&_svg:not([class*='text-'])]:text-muted-foreground focus-visible:border-primary focus-visible:ring-primary/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:bg-input/30 dark:hover:bg-input/50 flex w-fit items-center justify-between gap-2 rounded-xl border bg-slate-50/50 text-sm whitespace-nowrap text-slate-800 shadow-xs transition-all duration-200 outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 md:text-sm dark:text-white *:data-[slot=select-value]:dark:text-white [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
...
function SelectWithOptions({
  ...
  // Normalize value: convert empty string, null, or undefined to "all" or undefined
  // Radix Select needs undefined for uncontrolled or a valid string value
  const normalizedValue = React.useMemo(() => {
    if (value === null || value === undefined || value === "") {
      return showAllOption ? allOptionValue : undefined;
    }
    return String(value);
  }, [value, showAllOption, allOptionValue]);

  const handleValueChange = React.useCallback(
    (newValue: string) => {
      console.log("[SelectWithOptions] Value changed:", newValue);
      if (onValueChange) {
        console.log("[SelectWithOptions] Calling onValueChange");
        onValueChange(newValue);
      } else {
        console.warn("[SelectWithOptions] onValueChange is not provided!");
      }
    },
    [onValueChange],
  );
